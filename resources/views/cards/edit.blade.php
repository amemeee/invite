...
<head>
    ...
    <title>Edit — {{ $card->title }} — InviteMe</title>
    ...
    <header>
        <div class="header-inner">
            ...
            <div class="header-right">
                <a href="{{ route('cards.show', $card) }}" class="back-btn"><i class="bi bi-arrow-left"></i> View Card</a>
                <a href="{{ route('cards.manage', $card) }}" class="back-btn"><i class="bi bi-sliders"></i> Manage</a>
            </div>
        </div>
    </header>

    <div class="page">
        <div class="page-title">Edit Card</div>
        <p class="page-sub">Editing: {{ $card->title }}</p>
        ...
        <div class="glass-card">
            <form action="{{ route('cards.update', $card->id) }}" method="POST">
                ...
                <div class="field-group">
                    ...
                    <input type="text" class="form-control" name="title" value="{{ old('title', $card->title) }}" placeholder="e.g. Sarah & John Wedding">
                    @error('title')<div class="error-msg">{{ $message }}</div>@enderror
                </div>

                <div class="field-group">
                    <label class="form-label">Message</label>
                    <textarea class="form-control" name="message" id="message" rows="6">{{ old('message', $card->message) }}</textarea>
                    @error('message')<div class="error-msg">{{ $message }}</div>@enderror
                </div>

                <div class="field-group">
                    <label class="form-label">Event Date &amp; Time</label>
                    <input type="datetime-local" class="form-control" name="event_date" value="{{ old('event_date', $card->event_date?->format('Y-m-d\TH:i')) }}">
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('cards.show', $card) }}" class="btn-reset"><i class="bi bi-x"></i> Cancel</a>
                </div>
            </form>
        </div>
    </div>
